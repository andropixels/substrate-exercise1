...
pub mod pallet {
    ...
    pub type InvitaionId<T>=<T as orml_nft::Config>::TokenId;
	pub type CustomId<T> = <T as orml_nft::Config>::TokenId;
	///getiinvitationId
      
// 	#[pallet::metadata(
// 		T::KittyIndexOf<T> = "KittyIndex"
// 	)]
// struct test{
// 	kittyid:KittyIndexOf<T>,
// }
// #[derive(Clone, Encode, Decode, PartialEq, RuntimeDebug, TypeInfo)]
// #[scale_info(skip_type_params(T))]
// pub struct test<T: Config>{
...
// }
	...
	#[pallet::metadata(
		T::CustomId<T>="CustomId",T::AccountId = "AccountId", T::KittyIndexOf<T> = "KittyIndex",T::InvitaionId<T>="InvitationId", Option<BalanceOf<T>> = "Option<Balance>", BalanceOf<T> = "Balance",
	)]
	pub enum Event<T: Config> {
		/// A kitty is created. \[owner, kitty_id, kitty\]
		// KittyCrNFTcreatedount, KittyIndexOf<=T>, Kitty),
		NFTCreated(T::AccountId, KittyIndexOf<T>),
		...
		NFTTransfered(T::AccountId, T::AccountId, KittyIndexOf<T>),
		/// The price for a kitty is updated. \[owner, kitty_id, price\]
		NFTPriceUpdated(T::AccountId, KittyIndexOf<T>, Option<BalanceOf<T>>),
		/// A kitty is sold. \[old_owner, new_owner, kitty_id, price\]
		NFTSold(T::AccountId, T::AccountId, KittyIndexOf<T>, BalanceOf<T>),
		Test(T::AccountId),
		InvitaionCreated(T::AccountId,InvitaionId<T>),
		CustomCreated(T::AccountId ),
	}
	...
	impl<T:Config> Pallet<T> {
		...
		pub fn test(origin:OriginFor<T>) -> DispatchResult{
			...
			Self::deposit_event(Event::Test(sender));
			Ok(())
		...
		///create avanger nft
		#[pallet::weight(10_000)]
          pub fn create_invitation(origin:OriginFor<T>) -> DispatchResult{

			let sender = ensure_signed(origin)?;
			let time = Self::random_value(&sender);
			let _invitation= Invitation(time);
			let dna = Self::random_value(&sender);

			// Create and store kitty
			let kitty = Kitty(dna);
			let invitation_id = orml_nft::Pallet::<T>::mint(&sender, Self::class_id(),Vec::new(),kitty.clone())?;
			log::info!("A kitty is born with ID: {:?}.", invitation_id);
			Self::deposit_event(Event::InvitaionCreated(sender,invitation_id));
			Ok(())
            
		  }

		  ///create custom marrage NFT
           #[pallet::weight(10_000)]
             pub fn custom_marrage_nft<'a>(origin:OriginFor<T>, groom:u8,bride:u8,cid:u32)-> DispatchResult{
                ...
                let sender =ensure_signed(origin)?;
					...
					// Create and store kitty
					let kitty = Kitty(dna);
				let nft_id = orml_nft::Pallet::<T>::mint(&sender,Self::class_id(),Vec::new(),kitty.clone())?;
				// let tes= test{
				// 	kittyid:nft_id,
				// };
				log::info!("A kitty is born with ID: {:?}.", nft_id);
				Self::deposit_event(Event::CustomCreated(sender));
				Ok(())
		...
		pub fn create(origin: OriginFor<T>) -> DispatchResult {
			...
			// Emit event
			Self::deposit_event(Event::NFTCreated(sender, kitty_id));
		...
		pub fn transfer(origin: OriginFor<T>, to: T::AccountId, kitty_id: KittyIndexOf<T>) -> DispatchResult {
			...
			if sender != to {
				...
				Self::deposit_event(Event::NFTTransfered(sender, to, kitty_id));
			}
		...
		pub fn set_price(origin: OriginFor<T>, kitty_id: KittyIndexOf<T>, new_price: Option<BalanceOf<T>>) -> DispatchResult {
			...
			Self::deposit_event(Event::NFTPriceUpdated(sender, kitty_id, new_price));
		...
		pub fn buy(origin: OriginFor<T>, owner: T::AccountId, kitty_id: KittyIndexOf<T>, max_price: BalanceOf<T>) -> DispatchResult {
			...
			KittyPrices::<T>::try_mutate_exists(kitty_id, |price| -> DispatchResult {
				...
				Self::deposit_event(Event::NFTSold(owner, sender, kitty_id, price));
